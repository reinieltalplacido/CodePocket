...
import { groupCache, authCache } from "@/lib/cache-utils";
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check group cache
    const cacheKey = `groups:list:${user.id}`;
    ...
    const { data: groups, error } = await serviceClient
      ...
      .or(
        `owner_id.eq.${user.id}${memberGroupIds.length > 0 ? `,id.in.(${memberGroupIds.join(",")})` : ""}`,
      )
    ...
    // Cache the result
    groupCache.set(cacheKey, groupsWithOwnership);
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Invalidate group cache for this user
