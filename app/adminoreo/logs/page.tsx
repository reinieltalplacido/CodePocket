...
export default function AdminLogsPage() {
  ...
  useEffect(() => {
    ...
    setCurrentPage(1);
    setHasMore(true);
    fetchAnalytics(password, 1, true);
  }, [router, filter, dateRange, customStartDate, customEndDate]);
  ...
  const fetchAnalytics = async (password: string, page = currentPage, resetLogs = false) => {
    if (page === 1 && !resetLogs) {
      setLoading(true);
    } else {
      setLoadingMore(true);
    ...
    try {
      ...
      if (resetLogs || page === 1) {
        ...
        setAllLogs(prev => [...prev, ...data.recentLogs]);
      }
    ...
    const observer = new IntersectionObserver(
      entries => {
        if (entries[0].isIntersecting && hasMore && !loadingMore && !loading) {
          ...
          fetchAnalytics(password, nextPage);
        }
      },
      { threshold: 0.1 }
    );
  ...
  // Auto-refresh stats every 30 seconds (not logs to avoid disrupting scroll)
  useEffect(() => {
    const password = sessionStorage.getItem("admin_password");
    if (!password) return;
    ...
    const interval = setInterval(() => {
      ...
      fetchAnalytics(password, 1, true).then(() => setIsRefreshing(false));
    }, 30000);

    return () => clearInterval(interval);
  }, [filter, dateRange, customStartDate, customEndDate]);
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mb-6 flex flex-wrap gap-4">
            ...
            <div className="flex items-center gap-2 text-sm text-slate-400">
              <span>{allLogs.length} logs loaded</span>
          ...
          <div ref={observerTarget} className="mt-6 flex items-center justify-center py-8">
            {loadingMore && (
            ...
            {!hasMore && allLogs.length > 0 && (
              <div className="text-sm text-slate-500">
                No more logs to load • Total: {analytics?.pagination.totalItems || allLogs.length}
              </div>
            )}
          </div>
        </div>
